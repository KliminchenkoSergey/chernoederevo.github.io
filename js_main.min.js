...
function t456_highlight() {
  ...
  var shouldBeActiveElements = document.querySelectorAll('.t456__list_item a[href=\'' + url + '\'], ' + '.t456__list_item a[href=\'' + url + '/\'], ' + '.t456__list_item a[href=\'' + pathname + '\'], ' + '.t456__list_item a[href=\'/' + pathname + '\'], ' + '.t456__list_item a[href=\'' + pathname + '/\'], ' + '.t456__list_item a[href=\'/' + pathname + '/\']');
...
function t456_checkAnchorLinks(recid) {
  ...
  var rec = document.getElementById('rec' + recid);
  var navLinks = rec ? rec.querySelectorAll('.t456__list_item a[href*=\'#\']') : [];
...
function t456_catchScroll(navLinks) {
  ...
  t456_highlightNavLinks(navLinks, sections, sectionToNavigationLinkID, clickedSectionID);
  ...
  window.addEventListener('scroll', function () {
    ...
    if (lastCall && dateNow < lastCall + interval) {
      ...
      timeoutID = setTimeout(function () {
        lastCall = dateNow;
        clickedSectionID = t456_highlightNavLinks(navLinks, sections, sectionToNavigationLinkID, clickedSectionID)
...
function t456_highlightNavLinks(navLinks, sections, sectionToNavigationLinkID, clickedSectionID) {
...
function t456_fadeIn(element, duration, callback) {
  if (!element) return !1;
  if ((getComputedStyle(element).opacity === '1' || getComputedStyle(element).opacity === '') && getComputedStyle(element).display !== 'none') return !1;
  var opacity = 0;
  duration = parseInt(duration, 10);
  var speed = duration > 0 ? duration / 10 : 40;
  element.style.opacity = opacity;
